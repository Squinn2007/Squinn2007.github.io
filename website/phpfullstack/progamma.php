...
<body>
    ...
    <form method="post" action="">
        ...
        <select name="band_ids[]" multiple required> <!-- Multi-select toegevoegd -->
            ...
            while ($band = $bandsQuery->fetch_assoc()) {
                echo "<option value='" . $band['idBands'] . "'>" . htmlspecialchars($band['bandname']) . "</option>";
            }
            ?>
        </select>
    ...
    <div id="results">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_POST['reset'])) {
                ...
                if ($checkResult->num_rows > 0) {
                    echo "<p style='color:red;'>Het tijdstip $hoelaat bestaat al. Kies een ander tijdstip.</p>";
                } else {
        ...
        // Query om data uit beide tabellen te halen en te koppelen
        $query = $conn->query("
            SELECT Bands.bandname, Bands.genre, Events.naam AS eventnaam, Events.hoelaat, Events.datum, Events.prijs
        ...
        if ($query->num_rows > 0) {
            ...
            while ($row = $query->fetch_assoc()) {
                ...
                echo "<td>" . htmlspecialchars($row['bandname']) . "</td>";   // Weergave van de bandnaam
                echo "<td>" . htmlspecialchars($row['genre']) . "</td>";      // Weergave van het genre
